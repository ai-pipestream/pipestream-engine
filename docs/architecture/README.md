...
| [08: Kafka Sidecar](./08-kafka-sidecar.md) | Offloading Kafka complexity and lease management to specialized sidecars. |
| [09: Scaling Model](./09-scaling-model.md) | Hybrid scaling strategies for engines (HPA) and modules (KEDA). |
...
```mermaid
graph TD
    Intake[Intake Service] -- gRPC --> Engine[Pipestream Engine]
    Engine -- gRPC --> Module[Remote Module]
    
    subgraph ComputeTask [Compute Task Pod]
        Engine
    ...
    Engine -- Persist --> Repo[(Repo Service)]
    Repo --- S3[(S3 Storage)]
```

### Key Design Principles

*   **Pure gRPC Engine**: The engine has no knowledge of Kafka or message brokers. It only receives and sends gRPC requests, making it stateless and easy to scale.
*   **Sidecar Bridge**: The Kafka Sidecar handles the complexity of offset management, topic leases, and document hydration before delivering work to the engine.
